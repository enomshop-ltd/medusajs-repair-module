...
export const addRepairPartsStep = createStep(
  ...
  async (input: AddRepairPartsInput, { container }) => {
    ...
    if (input.customer_id && input.region_id) {
      ...
      const priceMetadata = variantsWithPrices?.reduce((acc: any, variant: any) => {
        acc[variant.id] = {
    ...
    return new StepResponse(
      { repair_ticket_id: input.repair_ticket_id, variant_ids: input.variant_ids },
      linkData
    )
  },
