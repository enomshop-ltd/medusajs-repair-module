...
export const updateRepairCostsStep = createStep(
  ...
  async (input: UpdateRepairCostsInput, { container }) => {
    ...
    if (input.parts_estimate !== undefined) {
      ...
      updateData.total_estimate = input.parts_estimate + (input.labor_estimate ?? currentTicket.labor_estimate)
    }

    if (input.labor_estimate !== undefined) {
      updateData.labor_estimate = input.labor_estimate
      updateData.total_estimate = (input.parts_estimate ?? currentTicket.parts_estimate) + input.labor_estimate
    }

    if (input.parts_actual !== undefined) {
      updateData.parts_actual = input.parts_actual
      updateData.total_actual = input.parts_actual + (input.labor_actual ?? currentTicket.labor_actual)
    }

    if (input.labor_actual !== undefined) {
      updateData.labor_actual = input.labor_actual
      updateData.total_actual = (input.parts_actual ?? currentTicket.parts_actual) + input.labor_actual
    }
